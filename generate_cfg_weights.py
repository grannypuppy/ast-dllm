...
from cfg_dag import generate_token_weights_from_cfg_dag
from tqdm import tqdm

def process_dataset(input_path: str, tokenizer):
    """
    Processes a .jsonl dataset file to add token weights based on CFG DAG to each entry.

    For each line in the input file, it parses the JSON, retrieves the 'input' code,
    generates token weights using the provided tokenizer and `cfg_dag` logic,
    ...
    output_path = input_path.replace('.jsonl', '_with_cfg_weights.jsonl')
    ...
    try:
        with open(input_path, 'r', encoding='utf-8') as infile, \
            ...
            num_lines = sum(1 for line in open(input_path, 'r', encoding='utf-8'))
            infile.seek(0)

            for line in tqdm(infile, total=num_lines, desc=f"Processing {os.path.basename(input_path)}"):
                try:
                    ...
                    if code and isinstance(code, str):
                        ...
                        tokens, _, depths, _ = generate_token_weights_from_cfg_dag(code, tokenizer)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate token weights for code datasets using CFG-DAG.")
    parser.add_argument(
        '--files', 
        nargs='+', 
    ...
    try:
        # Initialize the tokenizer, consistent with cfg_dag.py
